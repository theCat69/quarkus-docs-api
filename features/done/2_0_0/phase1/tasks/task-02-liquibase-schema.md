# Task 02: Liquibase Schema (PostgreSQL DDL)

> **Dependencies**: Task 01 (dependencies & configuration must be in place).

## Summary

Replace the contents of `src/main/resources/db/scripts/init.sql` with PostgreSQL-compatible DDL for the existing 8-table schema. The current file contains a Phase 2 `doc_chunks` table — discard it entirely. The Liquibase master changelog (`db/postgres.yaml`) already includes all scripts in the `scripts/` directory, so Liquibase will pick up the rewritten file automatically with `migrate-at-start=true`.

## Changes

### `src/main/resources/db/scripts/init.sql` (rewritten)
...
- [ ] `init.sql` contains valid PostgreSQL DDL for all 8 tables with correct types (`BIGSERIAL`, `BIGINT`)
- [ ] All 11 indexes are created
- [ ] All foreign keys with `ON DELETE CASCADE` are present
- [ ] Liquibase `migrate-at-start=true` creates the schema on application startup
- [ ] No SQLite-specific syntax (`AUTOINCREMENT`, `PRAGMA`) in any SQL file

## Files Modified

- `src/main/resources/db/scripts/init.sql`
