...
- [ ] Add unit tests for `Stemmer.stem()`: "configuration" → "configur", "security" → "secur", "running" → "run" (via -ing strip + duplicate consonant reduction), "classes" → "class", "used" → "used" (remaining length after -ed strip would be 2, below minimum 3, so no strip), short words unchanged.
- [ ] Add unit tests for stemmer edge cases: words shorter than suffix, words that don't match any rule, words already stemmed, trailing duplicate consonant reduction (e.g., "stopping" → "stop").
- [ ] Implement `Stemmer` utility class in `com.fvd.common` with suffix-stripping rules and trailing duplicate consonant reduction.
